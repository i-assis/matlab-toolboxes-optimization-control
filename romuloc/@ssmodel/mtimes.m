...
% SSMODEL/MTIMES - Multiply two systems
%   
% out = mtimes( in1 , in2 , name ); 	
%       _____                  _____
% zd1<-|     |<- wd1    zd2 <-|     |<- wd2
% z1 <-| in1 |<- w1      z2 <-| in2 |<- w2
...
% y =  y1           <-|_____|<- u =  u2
%	               
% Either <in1> or <in2> can be matrices or LTI objects.
% If <in1> and <in2> are arrays, 'mtimes' is performed element-wise. 
% If only one is an array, 'mtimes' is performed for all its elements.
%
...
%   LAAS-CNRS, Toulouse, France
  ...
  elseif ~isa(in2,'ssmodel')
    ...
    if length(in1.c{4})~=length(in2.r{4})
      error('signal dimensions do not fit');
    end    
%    if any(in1.c{3}) & any(in2.c{3})
%      error('input args. cannot have simultaneously <w> performance inputs');
%    end  
%    if any(in1.r{3}) & any(in2.r{3})
%      error('input args. cannot have simultaneously <z> performance outputs');
%    end    
    ...
    for ii=1:modnb
      ...
      out.M(m2r,m1c,ii) = in1.M(tr1,c1,i1)*in2.M(r2,tc2,i2);
