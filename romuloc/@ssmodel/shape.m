function out = shape(in1,in2,name)
% SSMODEL/SHAPE - Add filter on the performance transfer
%   
% out = shape( in1 , in2 , name ); 	
%       _____                  _____
% zd1<-|     |<- wd1    zd2 <-|     |<- wd2
% z1 <-| in1 |<------ w1=z2 <-| in2 |<- w2
% y1 <-|_____|<- u1      y2 <-|_____|<- u2
%        
% gives the augmented system with concatenated outputs/inputs
%                      _____
% zd=[ zd2' zd1' ]' <-|     |<- wd=[ wd2' wd1' ]'
% z =  z2           <-| out |<- w =  w1
% y =[ y2'  y1'  ]' <-|_____|<- u =[ u2'  u1'  ]'
%	               
% Either <in1> or <in2> can be matrices.
% If <in1> and <in2> are arrays, 'shape' is performed element-wise. 
% If only one is an array, 'shape' is performed for all its elements.
%
% <name> is an optionnal string to give a name to the resulting system   
%  
% SEE ALSO certain, feedback, ssmodel   
...
%   LAAS-CNRS, Toulouse, France
  ...
  elseif ~isa(in2,'ssmodel')
    ...
    if length(in1.c{3})~=length(in2.r{3})
      error('performance signal dimensions do not fit');
    end    
    ...
    tr1  = [in1.r{1} in1.r{2} in1.r{4}];
    ltr1 = length(tr1);
    tr2  = [in2.r{1} in2.r{2} in2.r{4}];
    ltr2 = length(tr2);
    tc1  = [in1.c{1} in1.c{2} in1.c{4}];
    ltc1 = length(tc1);
    tc2  = [in2.c{1} in2.c{2} in2.c{4}];
    ltc2 = length(tc2);
    ...
    for ii=1:modnb
      ...
      out.M(m2r,m1c,i1) = in1.M(tr1,c1,i1)*in2.M(r2,tc2,i2);
      out.M(m2r,m2c,ii) = in1.M(tr1,tc1,i1);
      out.M(m2r,m3c,ii) = in1.M(tr1,c1,i1)*in2.M(r2,c2,i2);
      
      out.M(m3r,m1c,ii) = in1.M(r1,c1,i1)*in2.M(r2,tc2,i2);
    ...
    out.r{3} = (1+lastr1):(lastr1+lr1);
    out.c{3} = (1+lastc1):(lastc1+lc2);
